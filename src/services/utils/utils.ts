...
export const AddToCacheData = async (cacheName: string, name: string, dataName: string) => {
  try {
...
export const getCitiesWeatherFromCache = async (cacheName: string, url: string) => {
  // get data from cache
  try {
    const cache = await caches.open(cacheName)
